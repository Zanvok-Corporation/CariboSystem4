...
; Compile for PC DOS, .com or boot load, or CariboSystem (0/1)
CariboSystem equ 1
Def_Com equ 0
...
%endmacro
				...
				; One memory segment for .com, 1/2 segment for CariboSystem
				; future - consider separate stack and/or disk buffer seg
%if CariboSystem
	%include "caribodev.inc"
	org	0x8000
%else
%if Def_Com
	org	0x0100			; .com
...
; Disk input can start anywhere (CariboSystem) or Buf0 (DOS)
HEADING 'TIB'			; ( -- tib )
  TIB:		dw create
  tib1:		dw stack0
...
%if CariboSystem
  ...
  c_seg		dw	0
  sp_save	dw	0	; save CariboSystem SS:SP
  ss_save	dw	0
%endif
...
; CariboSystem will read whole file; make sure it will fit!
HEADING 'FREAD'
...
; CariboSystem will not overwrite an existing file
HEADING 'FWRITE'
...
%if CariboSystem
; CariboSystem get file size to ensure it will fit in available memory
HEADING 'F_SIZE'
...
; CariboSystem read entire file - it must fit in memory without hurting CariboSystem or Forth
...
; Note - no address can be less than 0100h (.com) or 8000h (CariboSystem)
...
; LFA since > 0100h (DOS) or 0x8000 (CariboSystem), can be used as "found" flag
...
; CariboSystem is not a good fit for nesting.
HEADING "INCLUDE"		; ( -- )
		dw colon
...
HEADING 'D_VER'			; DOS or CariboSystem API version - display purposes
  D_VER:        dw create
  dver1:        dw      0
...
start_forth:
	...
	db      sf01 - $ - 1
%if CariboSystem
	db      'Copyright (C) 2018-2022 Zanvok Corporation -- see doc/LICENSE.TXT'
%else
